...
- 多跳链路：入口 → 中转 → … → 目标，监听端口留空就从节点配的范围里随机分配
- 一键链路测试：逐跳让每台机器去拨自己的下一跳，直接告诉你断在第几段，
...
- **云服务商的安全组/防火墙管不了**。转发用的入口端口还得你自己去服务商控制台放行，
  那一层在机器外面。链路测试里「面板 → 入口」这一段不通、但每一跳都通，
  基本就是这个原因。
