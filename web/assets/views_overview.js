...
export const NodeDetailView = {
    ...
    template: `
        <div v-if="node">
            ...
            <!-- 只在这台机器真的在转发时才显示，别给没用转发的用户增加噪音。 -->
            <div v-if="node.forward_share > 0" class="notice" style="margin-bottom:16px">
                本周期用掉的 {{ fmtBytes(node.quota_status.billed_bytes) }} 里，
                <b>{{ fmtBytes(node.forward_share) }}</b> 是端口转发消耗的，
                另外 <b>{{ fmtBytes(Math.max(0, node.quota_status.billed_bytes - node.forward_share)) }}</b>
                是这台机器自己用掉的。
            </div>
